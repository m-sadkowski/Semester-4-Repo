function [ft_5, integral_1000, Nt, integration_error] = simpson_rule_accuracy_evaluation()
    ...
    title('Błąd całkowania a liczba podprzedziałów (Metoda Simpsona)');
    grid on;
end
...
function integral_approximation = simpson_rule(x)
    ...
    for i = 1:length(x)-1
        ...
        fab2 = failure_density_function((a + b)/2);
        first = (b - a)/6;
        second = fa + 4 * fab2 + fb;
        integral_approximation = integral_approximation + first * second;
    end
end
